...
Les éléments suivants sont requis pour chaque exercice.
...
Cet exemple montre comment créer un conteneur nommé `iisbase`, puis démarrer une session interactive avec le conteneur.
...
Copiez le texte suivant dans le fichier dockerfile et enregistrez-le. Ces commandes indiquent à Docker de créer une image en se servant de `windosservercore` comme base, puis d’inclure les modifications spécifiées avec `RUN`. Pour plus d’informations sur les fichiers Dockerfile, voir les [informations de référence sur Docker Run sur docker.com](http://docs.docker.com/engine/reference/builder/).
...
Dans cet exemple, un répertoire de l’hôte du conteneur est rendu accessible au conteneur en cours d’exécution à l’aide du paramètre `-v` de `docker run`. Avant cela, le répertoire source doit être configuré.
